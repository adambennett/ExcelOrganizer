    ...
    private static List<List<LoanDataValuesDTO>> groupSortMultiRowReportData(List<List<List<LoanDataValuesDTO>>> data) {
        ...
        for (int rowIndex = 0; rowIndex < rows; rowIndex++) {
            ...
            for (int columnIndex = 0; columnIndex < columns; columnIndex++) {
                ...
                if (groupMatches.stream().anyMatch(g -> g.key().equalsIgnoreCase(currentGroupKeyForGroupingKey.getOrDefault(header.groupingKey(), null)))) {
                    ...
                    continue;
                } else if (currentIterationIndexOfHeader.get(header.identifier()) >= groupMatches.size()) {
                    ...
                    currentGroupKeyForGroupingKey.put(header.groupingKey(), correctCurrentGroupKeyForGroupingKey.get(header.groupingKey()));
